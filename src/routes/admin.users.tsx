...
function UsuariosPage() {
  ...
  const [pwUsuario, setPwUsuario] = useState<typeof USUARIOS_INICIALES[number] | null>(null);
  ...
  const guardarPassword = () => {
    ...
    registrarAuditoria("Carlos Mendoza (Admin)", `Cambio de contraseña para: ${pwUsuario.nombre}`, "Seguridad");
    toast.success(`Contraseña actualizada para ${pwUsuario.nombre}. Acción auditada.`);
    setPwUsuario(null);
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <span className="font-semibold text-foreground">{pwUsuario?.nombre}</span>. El usuario deberá usar esta nueva contraseña en su próximo ingreso.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            ...
            <p className="rounded-lg bg-muted/50 px-3 py-2 text-xs text-muted-foreground">
              La acción quedará registrada en el log de auditoría con tu usuario administrador.
            </p>
          </div>

          <DialogFooter className="gap-2 sm:gap-2">
            <Button variant="outline" onClick={() => setPwUsuario(null)} className="h-10 rounded-lg">
              Cancelar
            </Button>
            <Button onClick={guardarPassword} className="h-10 gap-2 rounded-lg font-semibold">
